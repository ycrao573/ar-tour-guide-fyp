...
var World = {
  ...
  loadPoisFromJsonData: function loadPoisFromJsonDataFn(poiData) {
    ...
    for (
      ...
      var singlePoi = {
        id: poiData[currentPlaceNr].id.toString(),
        latitude: parseFloat(poiData[currentPlaceNr].latitude),
        longitude: parseFloat(poiData[currentPlaceNr].longtitude),
        altitude: 100.0,
        title: poiData[currentPlaceNr].name,
        description: poiData[currentPlaceNr].description,
        category: poiData[currentPlaceNr].category,
        officiallink: poiData[currentPlaceNr].Field_1,
        hyperlink: extractContent(poiData[currentPlaceNr].HYPERLINK),
        urlpath: extractContent(poiData[currentPlaceNr]["URL Path"]),
        photourl:
      ...
      World.markerDrawableImage = new AR.ImageResource(
        singlePoi.photourl.startsWith("https://")
          ? singlePoi.photourl
          : "https://" + singlePoi.photourl, {
        onError: World.onError
    });
    ...
    const defaultDisplayDistance = 6500;
    var defaultDisplayPosition =
      maxPlaceDistance > defaultDisplayDistance
        ? Math.round(maxPlaceDistance / defaultDisplayDistance)
        : 100;

    document.getElementById("panelRangeSliderValue").innerHTML =
  ...
  onMarkerSelected: function onMarkerSelectedFn(marker) {
    ...
    //   "https://maps.googleapis.com/maps/api/staticmap?center=" + marker.poiData.title + " Singapore"
    //   "&markers=" + marker.poiData.title + " Singapore" + "&zoom=14&size=400x400&key=AIzaSyCcuOYBEHg6xRvC-NU-ScSPH01aDndnV_w";
    // }
    // else {
    //   document.getElementById("poiDetailImage").src = "https://" + marker.poiData.photoUrl;
    // }
  ...
  updateRangeValues: function updateRangeValuesFn() {
    ...
    document.getElementById("panelRangePlaces").innerHTML =
      placesInRange > 1 ? placesInRange + " Places" : placesInRange + " Place";
    document.getElementById("panelRangeSliderValue").innerHTML = slider_value;

    World.updateStatusMessage(
      placesInRange > 1
        ? placesInRange + " places loaded"
        : placesInRange + " place loaded"
    );
  ...
  getNumberOfVisiblePlacesInRange: function getNumberOfVisiblePlacesInRangeFn(
    maxRangeMeters
  ) {
